...
class ModelPerformanceDegraded(DomainEvent):
    """Event fired when model performance drops below threshold."""
    ...
    def __post_init__(self):
        """Validate and set aggregate_id."""
        if not (0.0 <= self.current_accuracy <= 1.0):
            raise ValueError("Current accuracy must be between 0.0 and 1.0")
        if not (0.0 <= self.threshold_accuracy <= 1.0):
            raise ValueError("Threshold accuracy must be between 0.0 and 1.0")
        if self.degradation_percentage < 0:
            raise ValueError("Degradation percentage cannot be negative")
        object.__setattr__(self, "aggregate_id", self.model_id)
        super().__post_init__()
